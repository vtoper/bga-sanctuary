...
class DrawTile extends AutomaticActionState
{
    ...
    public function getCustomStateDescription()
    {
        if (!is_null($this->getNodeArgs("sourceName"))) {
            return [
                "description" => clienttranslate('${actplayer} must draw a tile (${sourceName})'),
                "descriptionMyTurn" => clienttranslate('${you} must draw a tile (${sourceName})'),
            ];
        }
        return null;
    }
    ...
    public function getDescription()
    {
        if (!is_null($this->getNodeArgs("sourceName"))) {
            return [
                "log" => clienttranslate('Draw a tile (${sourceName})'),
                "args" => [
                    "sourceName" => $this->getNodeArgs("sourceName", "")
                ]
            ];
        }
        return clienttranslate('Draw a tile');
    }
