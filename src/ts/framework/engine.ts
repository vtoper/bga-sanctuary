...
export const showEngine = async (engine: EngineNode) => {
  ...
  const closeNode = createElement(`<i class="fa fa-times fa-3x absolute top-2 left-2"></i>`);
  ...
  const nodesContainer = createDivElement(
    getRandomId('engine-nodes-container'),
    'flex flex-col items-center-safe justify-center-safe overflow-auto pt-5',
  );
...
const createEngineHeader = (node: EngineNode) => {
  const headerElement = createDivElement(
    getRandomId('engine-header'),
    'flex flex-row items-center-safe justify-center-safe mb-3',
  );
...
const createEngineNode = (node: EngineNode) => {
  const nodeElement = createDivElement(
    getRandomId('engine-node'),
    'flex flex-col items-center-safe justify-center-safe rounded-md border border-white border-solid p-2',
  );
  ...
  const childrenElement = createDivElement(
    getRandomId('engine-children'),
    'flex flex-row items-center-safe justify-center-safe gap-3',
  );
