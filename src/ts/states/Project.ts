...
export class Project {
  ...
  private args: ProjectArgs | null = null;
   ...
   */
  onEnteringState(args: ProjectArgs, isCurrentPlayerActive: boolean) {
    this.args = args;
    this.resetSelection();
  ...
  private makeHandSelectable() {
    for (const tileId of players.getHandTileIds()) {
      ...
      if (tileId === this.selectedTileId) {
        ...
        onClick(node, () => this.selectProject(null));
      } else if (this.args.playableTiles[tileId]) {
        onClick(node, () => this.selectProject(tileId));
      }
    }
  }
  ...
  private selectProject(tileId: string | null) {
    this.resetSelection();
